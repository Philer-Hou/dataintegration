...
import com.data.cas.server.common.captcha.CaptchaUtils;
import com.data.cas.server.service.AuthService;
import com.data.common.config.Config;
import com.data.common.constant.Constant;
import com.data.common.json.CResult;
import com.data.common.status.ResultStatus;
import com.data.pmodel.LoginInfo;
...
public class AuthController {
	...
	@ExceptionHandler(value = Exception.class)
	public CResult<P_User> login(HttpServletRequest request, HttpServletResponse response,
			@RequestBody P_Auth auth) {
		// 判断登录结果
		...
		if (!CaptchaUtils.getInstance().checkVerifyCaptcha(request, auth.getCaptcha())) {
			...
			return new CResult<P_User>(success, null, null, null, null, message);
		}
		// 进行登录
		P_User user = authService.login(auth);
		if (user != null) {
			// 记录登录状态
		...
		return new CResult<P_User>(success, code, user, null, null, message);
	}
